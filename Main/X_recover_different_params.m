function [Results, title_cell] = X_recover_different_params(  params_struct, params_type, params_vec )
% Running signal recovery experiments at different scenarios
...
%% Running the signal recovery algorithms for Ntrials

N_params = length(params_vec);

RelativeEr_cell_all = cell(N_params, Ntrials);
SNR_cell_all = cell(N_params, Ntrials);
...
for n = 1:N_params
    ...
    for trial = 1:Ntrials
        ...
        [Result, title_cell] = X_recovery_algorithms( Y, data_struct, params_struct );
        
        
        
        % Save the results
        RelativeEr_cell_all{n, trial} = Result.RelativeEr;
        SNR_cell_all{n, trial} = Result.SNRs;
