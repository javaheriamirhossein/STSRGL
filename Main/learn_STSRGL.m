...
% Extract the lower diagonal indices of a symmetric hollow matrix 
% (used for Lalacian conjugate operator)
[~, lowerdiag_ind] = diag_lowerdiag_index(N);
Nw = size(lowerdiag_ind,1);
row_col_id = zeros(Nw,2);
for k=1:Nw
    [row_col_id(k,1), row_col_id(k,2)] =  id_2_row_col( lowerdiag_ind(k,1), N );
end
...
for i=1:maxIter
    ...
    if Compute_f
        ...
        r = conj_L_operator( K, row_col_id );
        f_joint = T*sum(w.*r);
    end
    ...
    if Optimize_L
        for loop = 1:Nloop
            ...
            q =  conj_L_operator( L_inv, row_col_id );
            zeta = tau.*w.^2;
