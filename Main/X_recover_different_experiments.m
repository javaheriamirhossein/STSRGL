function [ params_struct, all_results ] = X_recover_different_experiments( params_struct, experiments, savedir)
...
if nargin<3
    savedir = [cd, '\Results\Signal_Recovery\', date, '\'];
end
...
for n=1:Nexp
    ...
    switch experiment
        case 'noise'
            ...
            param_type = 'noise';
            [Results, title_cell] = X_recover_different_params(  params_struct, param_type, input_vec );            x_axis_log = 0;
            
        case 'sr'
            input_vec = linspace(0.1,0.9,10);
            labelx = 'Sampling Rate';        
            param_type = 'sr';
            [Results, title_cell] = X_recover_different_params(  params_struct, param_type, input_vec );
        ...
        case 'alpha_1'
            ...
            labelx = '$\alpha_1$';        
            param_type = 'alpha_1';
            [Results, title_cell] = X_recover_different_params(  params_struct, param_type, input_vec );            x_axis_log = 1;
            x_axis_log = 1;


    end

    [Results_mat_cell, field_names] = Results_cell2tensor( Results );

    titleStr = [task, ' under different ', experiment];

    legend_cell = title_cell;

    N_figs = length(Results_mat_cell);
    ylabls = {'NMSE', 'SNR', 'time (s)'};
    ...
    for k = 1:N_figs
        ...
        figsavename = [savedir, 'Figure Xrec ', description, ' diff ', experiment, ' ', figname, '.fig'];                 
        savefig(gcf,figsavename);
        figsavename = [savedir, 'Figure Xrec ', description, ' diff ', experiment, ' ', figname, '.png'];
...
matsavename = [savedir, 'Results all Xrec ', description, '.mat']; 
